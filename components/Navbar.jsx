                    ...
                    }
                    {
                        Object.entries(networks).map((network, index) => {
                            if (network[1].chainid == (Noto ? chainId : currentNetwork)) {
                                return <button key={index} onClick={() => { setIsNetworkOption(true) }} className="flex flex-row items-center font-semibold dark:text-gray-100 bg-indigo-200 dark:bg-indigo-900 rounded-2xl gap-x-1 h-10 w-32 pl-4">
