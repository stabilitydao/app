...
const phaseTasks = [
    ...
    },
    {
        ...
        "Stability Fund Development (decentralized Improvement fund)": false,
                            ...
                            return (
                                <div className="p-2" key={index}>
                                    <div className="flex flex-row mb-2">
                                        <BiCircle
                                            className="relative p-1 text-4xl text-indigo-500 bg-indigo-500 border-2 border-indigo-500 rounded-full bg-clip-content -left-4" />
                                        ...
                                        <ul className="text-lg font-semibold ">
                                            {
                                                Object.keys(phaseTasks[index]).map((task, value) => {
                                                    return phaseTasks[index][task] ? <li key={value}><TiTick className="inline pb-1 text-3xl text-teal-500" /><span dangerouslySetInnerHTML={{__html: task}} /></li> : <li className="pl-7" key={value}><span dangerouslySetInnerHTML={{__html: task}} /></li>
                                                })
                                            }
                                        </ul>
                                    </div>
