        ...
        <aside style={{ backgroundColor: Mode ? '#160024' : 'rgba(255,255,255,1)' }} className={`flex flex-col lg:h-screen justify-between overflow-y-auto fixed z-20 top-0 bottom-0 text-black bg-white shadow-900 dark:text-white w-72 xl:w-80 lg:static  duration-300  ${sidebar ? "left-0" : " -left-96"}`}>
            <div className="flex flex-col mb-8">
                ...
                <ul className="mt-1">
                    ...
                    <li><Link href="/profitmaker"><a className={`${activeRoute === "/profitmaker" ? "bg-indigo-600  text-white " : "text-black dark:text-white"} flex items-center py-2.5 text-xl pl-7 gap-x-2 `} onClick={() => { dispatch(updateSidebar(false)) }} ><BsPatchPlusFill className="mr-2" />ProfitMaker</a></Link></li>
                </ul>
                <ul className="mt-5">
